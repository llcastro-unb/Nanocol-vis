...
int iAgglParticle = 3; /* we are going to follow the agglomerate of this particle */

float sAggl = 1.1; /* surface distance to consider particles as agglomerated (twice the surfactant layer) */

float xfocus, yfocus, zfocus; /* point on which the camera will focus */
...
void display(void)
{
	...
	while(fscanf(fileParticles, " %f %f %f %f %f %f %f", &diam, &ry, &rz, &rx, &my, &mz, &mx)==7) /* x, y, z rodados */
	{
      ...
      getchar();
		...
		iParticle = iAgglParticle;
		
		xfocus = rxip[iParticle];
		yfocus = ryip[iParticle];
		zfocus = rzip[iParticle];
		
			for (jParticle = 0; jParticle < nParticles; jParticle++) 
			{
...
void reshape(int w, int h)
{
	...
	if (w <= h)
		glOrtho(-scale*Lx, scale*Lx, -scale*Ly, scale*Ly*h/w, -0.85e0*Lz, 0.65e0*Lz);
	else
		glOrtho(-scale*Lx, scale*Lx*w/h, -scale*Ly, scale*Ly, -0.85e0*Lz, 0.65e0*Lz);

	float eyeDistX,eyeDistY,eyeDistZ;
	float atDist;
	float eX,eY,eZ;
	float aX,aY,aZ;
	float dX,dY,dZ;
	...
	gluLookAt(eX,eY,eZ, aX,aY,aZ, -dY/(2.0e0*dX),1.0e0,-dY/(2.0e0*dZ));
	
/*	gluLookAt(eyeDist*xfocus*0.9e0,eyeDist*yfocus*0.8e0,eyeDist*zfocus,atDist*xfocus,atDist*yfocus,atDist*zfocus,0,1,0); */
