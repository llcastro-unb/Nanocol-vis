...
char fileBoxName[60] = "OUT-ConfiguracaoNM-box.txt";
char fileParticlesName[60] = "OUT-ConfiguracaoNM.txt";
	...
	while(fscanf(fileParticles, " %f %f %f %f %f %f %f", &diam, &ry, &rz, &rx, &my, &mz, &mx)==7) /* x, y, z rodados */
	{
      ...
      getchar();
		...
		for (iParticle = 0; iParticle < nParticles; iParticle++) 
		{
			...
			if(iParticle == 0)
			{
				printf("\n D, rx, ry, rz, mx, my, mz: %f %f %f %f %f %f %f", diam, ry, rz, rx, my, mz, mx);
			...
			else
			{
				if(fscanf(fileParticles, " %f %f %f %f %f %f %f", &diam, &ry, &rz, &rx, &my, &mz, &mx)==7) /* x, y, z rodados */
				{
					...
					if(polydisperse)
					{
						raio = diam / 2.0f;  /* For monodisperse system, we only read the first diameter. */
			                        printf("raio: %f",raio);
					}
				}
				else
